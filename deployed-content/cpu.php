          ...
          $display .= '<table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">';
            ...
            while($row = mysqli_fetch_array($result)){/*retrieving Data from*/
              $display .='<tr role="row" class="odd">';
                ...
                $display .='<td >';
                  $display .='<a data-trackerid="'.$row['row_id'].'" class="ui blue icon mini basic button" id = "btn_edititemdeployed"><i class="edit icon"></i> Edit</a>';
                  $display .='<a data-trackerid="'.$row['row_id'].'"  class="ui red icon mini basic button" id = "btn_deleteitemdeployed"><i class="trash icon"></i> Delete</a>';
                $display .='</td>';
              $display .='</tr>';
            }
            $display .= '</tbody>';
